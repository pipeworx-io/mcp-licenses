...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Open-source licenses');
}
...
async function getList(): Promise<Lic[]> {
  const res = await pwFetch(LIST, { headers: { Accept: 'application/json', 'User-Agent': UA } });
  if (!res.ok) throw await httpError(res, 'SPDX list error');
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    ...
    case 'get_license_text': {
      ...
      const res = await pwFetch(l.detailsUrl, { headers: { Accept: 'application/json', 'User-Agent': UA } });
      if (!res.ok) throw await httpError(res, 'SPDX details error');
